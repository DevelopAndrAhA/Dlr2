    ...
    List<MyOrder> myOrders  =(List<MyOrder>) ctx.getAttribute("myOrders");
     for(int i =0;i<myOrders.size();i++){
            ...
            out.print("<TD><INPUT style='display:none' TYPE=checkbox id='check"+i+"'></TD><TD style='display:none' id='orderId"+i+"'>"
                    ...
                    "</TD><TD><select class='selectCourier form-control'  name=product_category id='selectId"+i+"'><option>Назначить курьер</option></select></TD><TD class="+myOrders.get(i).getStatus()+">"
                    +myOrders.get(i).getStatus()+
                    "</TD><TD><a id='linkId"+i+"' href=detailsOrder?order="
                    +myOrders.get(i).getId()+
            ...
            <%
                if(sizeToPagination!=0){
                    if(pagesEnd){
                        ...
                        for(int k=1;k<=sizeToPagination;k++){
                            if(activePageNumber==k){
                                ...
                                out.print("<a class=page-link href=new_orders_p?page="+k+" tabindex=-1>"+k+"</a>");
                                out.print("</li>");
                            }else{
                    ...
                    if(activePageNumber<sizeToPagination){
                        ...
                        out.print("<a class=page-link href=new_orders_p?page="+nextpage+" tabindex=-1>След.</a>");
                        out.print("</li>");
                    }else{
